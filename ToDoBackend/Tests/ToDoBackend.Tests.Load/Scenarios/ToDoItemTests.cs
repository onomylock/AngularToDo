...
using ToDoBackend.Tests.Load.Halpers;
    ...
    [Test]
        public void ToDoItem_MultiStage_ClosedModel_Test()
        {
            ...
            .WithLoadSimulations(
                ...
                Simulation.RampingConstant( // Стадия 3: Пиковая нагрузка
                    copies: 0,
                    during: TimeSpan.FromMinutes(1)
                )
            );
        ...
        public void ToDoItem_Different_User_Loads_Test(int userCount)
        {
            ...
            var scenario = Scenario.Create($"ToDoItem_{userCount}Users_Test", async context =>
                {
                    var request = Http.CreateRequest("GET", _fullApi);
